...
# -------------------------- CSS and JavaScript for Blurring Effect -------------------------- #
st.markdown(
    """
    <style>
    /* CSS pour flouter et assombrir le contenu */
    .blurred {
        filter: blur(5px);
        opacity: 0.4;
        pointer-events: none;
    ...
    <script>
    // JavaScript pour ajuster les éléments visibles/masqués
    document.addEventListener('DOMContentLoaded', function() {
        // Trouver toutes les sections de contenu
        const sections = document.querySelectorAll('.stApp > div > section');
        
        // Assigner des classes pour appliquer le floutage
        sections.forEach((section) => {
            section.classList.add('blurred');
        });

        // Retirer l'effet flouté de la carte de France
        const mapSection = document.querySelector("h4:contains('📍 Map of France with Emissions by Department')");
        if (mapSection) {
            mapSection.parentElement.parentElement.id = 'france-map';
        }
    });
    </script>
    """,
